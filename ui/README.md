...
This directory contains the new single-page frontend shell for **玄武AI智能体股票团队分析系统**.

## Local development

The frontend is a standalone SPA under `ui/`, but it is designed to run next to the current Python service during migration.
...
The default development proxy points at the local API service on `8503`:

```env
VITE_APP_TITLE=玄武AI智能体股票团队分析系统
VITE_API_BASE=/api
VITE_API_PROXY_TARGET=http://127.0.0.1:8503
```
...
The frontend dev server runs on [http://127.0.0.1:4173](http://127.0.0.1:4173) and proxies backend calls to the Python API service.
