...
MONITORED_STOCKS_TABLE = "real_monitor_monitored_stocks"
PRICE_HISTORY_TABLE = "real_monitor_price_history"
NOTIFICATIONS_TABLE = "real_monitor_notifications"
...
class StockMonitorDatabase:
    ...
    def init_database(self):
        ...
        # 创建监测股票表
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS real_monitor_monitored_stocks (
        ...
        try:
            cursor.execute("SELECT trading_hours_only FROM real_monitor_monitored_stocks LIMIT 1")
        except sqlite3.OperationalError:
            cursor.execute("ALTER TABLE real_monitor_monitored_stocks ADD COLUMN trading_hours_only BOOLEAN DEFAULT TRUE")
            print("✅ 已添加trading_hours_only字段")

        # 创建价格历史表
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS real_monitor_price_history (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                stock_id INTEGER,
                price REAL NOT NULL,
                timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (stock_id) REFERENCES real_monitor_monitored_stocks (id)
            )
        ''')

        # 创建提醒记录表
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS real_monitor_notifications (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                stock_id INTEGER,
    ...
    def add_monitored_stock(self, symbol: str, name: str, rating: str,
        ...
        cursor.execute('''
            INSERT INTO real_monitor_monitored_stocks
    ...
    def get_monitored_stocks(self) -> List[Dict]:
        ...
        cursor.execute('''
            ...
            FROM real_monitor_monitored_stocks
    ...
    def update_stock_price(self, stock_id: int, price: float):
        ...
        # 更新当前价格
        cursor.execute('''
            UPDATE real_monitor_monitored_stocks
        ...
        # 记录价格历史
        cursor.execute('''
            INSERT INTO real_monitor_price_history (stock_id, price)
            VALUES (?, ?)
        ''', (stock_id, price))
    ...
    def add_notification(self, stock_id: int, notification_type: str, message: str):
        ...
        cursor.execute('''
            INSERT INTO real_monitor_notifications (stock_id, type, message)
            VALUES (?, ?, ?)
    ...
    def get_pending_notifications(self) -> List[Dict]:
        ...
        cursor.execute('''
            ...
            FROM real_monitor_notifications n
            JOIN real_monitor_monitored_stocks s ON n.stock_id = s.id
            WHERE n.sent = FALSE
    ...
    def mark_notification_sent(self, notification_id: int):
        ...
        cursor.execute('''
            UPDATE real_monitor_notifications SET sent = TRUE WHERE id = ?
        ''', (notification_id,))
    ...
    def mark_all_notifications_sent(self):
        ...
        cursor.execute('UPDATE real_monitor_notifications SET sent = TRUE WHERE sent = FALSE')
    ...
    def clear_all_notifications(self):
        ...
        cursor.execute('DELETE FROM real_monitor_notifications')
    ...
    def remove_monitored_stock(self, stock_id: int):
        ...
        try:
            ...
            # 删除相关记录
            cursor.execute('DELETE FROM real_monitor_price_history WHERE stock_id = ?', (stock_id,))
            cursor.execute('DELETE FROM real_monitor_notifications WHERE stock_id = ?', (stock_id,))
            cursor.execute('DELETE FROM real_monitor_monitored_stocks WHERE id = ?', (stock_id,))
    ...
    def get_stock_by_id(self, stock_id: int) -> Optional[Dict]:
        ...
        cursor.execute('''
            ...
            FROM real_monitor_monitored_stocks WHERE id = ?
        ''', (stock_id,))
    ...
    def get_monitor_by_code(self, symbol: str) -> Optional[Dict]:
        ...
        cursor.execute('''
            SELECT * FROM real_monitor_monitored_stocks WHERE symbol = ?
        ''', (symbol,))
