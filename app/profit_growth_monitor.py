...
MONITORED_STOCKS_TABLE = "profit_growth_monitored_stocks"
SELL_ALERTS_TABLE = "profit_growth_sell_alerts"
...
class ProfitGrowthMonitor:
    ...
    def _init_database(self):
        ...
        try:
            ...
            # 创建监控股票表
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS profit_growth_monitored_stocks (
            ...
            # 创建卖出提醒表
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS profit_growth_sell_alerts (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    stock_code TEXT NOT NULL,
                    stock_name TEXT NOT NULL,
    ...
    def add_stock(self, stock_code: str, stock_name: str, buy_price: float,
        ...
        try:
            ...
            # 检查是否已存在
            cursor.execute("""
                SELECT id FROM profit_growth_monitored_stocks
            ...
            # 插入新记录
            cursor.execute("""
                INSERT INTO profit_growth_monitored_stocks
    ...
    def get_monitoring_stocks(self) -> List[Dict]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT * FROM profit_growth_monitored_stocks
    ...
    def update_holding_days(self, stock_code: str, days: int) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                UPDATE profit_growth_monitored_stocks
                SET holding_days = ?
                WHERE stock_code = ? AND status = 'holding'
            """, (days, stock_code))
    ...
    def add_sell_alert(self, stock_code: str, stock_name: str, alert_type: str,
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO profit_growth_sell_alerts
    ...
    def get_unprocessed_alerts(self) -> List[Dict]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT * FROM profit_growth_sell_alerts
    ...
    def remove_stock(self, stock_code: str, reason: str = "手动移除") -> Tuple[bool, str]:
        ...
        try:
            ...
            cursor.execute("""
                DELETE FROM profit_growth_monitored_stocks
