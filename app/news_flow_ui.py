...
def display_dashboard():
    ...
    try:
        from app.news_flow_engine import news_flow_engine
    ...
    with col2:
        if st.button("📥 生成PDF报告", type="primary", key="gen_pdf_btn"):
            with st.spinner("正在生成PDF报告..."):
                try:
                    from app.news_flow_pdf import NewsFlowPDFGenerator
...
def display_alert_center():
    ...
    try:
        from app.news_flow_alert import news_flow_alert_system
        from app.news_flow_db import news_flow_db
    except Exception as e:
        st.error(f"模块加载失败: {e}")
        return
