...
MONITORED_STOCKS_TABLE = "low_price_bull_monitored_stocks"
SELL_ALERTS_TABLE = "low_price_bull_sell_alerts"
...
class LowPriceBullMonitor:
    ...
    def _init_database(self):
        ...
        # 创建监控列表表
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS low_price_bull_monitored_stocks (
        ...
        # 创建卖出提醒表
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS low_price_bull_sell_alerts (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                stock_code TEXT NOT NULL,
                stock_name TEXT NOT NULL,
    ...
    def add_stock(self, stock_code: str, stock_name: str, buy_price: float,
        ...
        try:
            ...
            # 检查是否已存在
            cursor.execute("""
                SELECT id FROM low_price_bull_monitored_stocks
            ...
            # 添加到监控列表
            cursor.execute("""
                INSERT INTO low_price_bull_monitored_stocks
    ...
    def remove_stock(self, stock_code: str, reason: str = "手动移除") -> Tuple[bool, str]:
        ...
        try:
            ...
            cursor.execute("""
                DELETE FROM low_price_bull_monitored_stocks
                WHERE stock_code = ? AND status = 'removed'
            """, (stock_code,))

            # 然后更新持仓中的记录为'removed'
            cursor.execute("""
                UPDATE low_price_bull_monitored_stocks
                SET status = 'removed',
                    remove_time = ?,
                    remove_reason = ?
                WHERE stock_code = ? AND status = 'holding'
    ...
    def get_monitored_stocks(self) -> List[Dict]:
        ...
        try:
            ...
            df = pd.read_sql_query("""
                SELECT * FROM low_price_bull_monitored_stocks
    ...
    def update_holding_days(self):
        ...
        try:
            ...
            # 获取所有持仓股票
            cursor.execute("""
                SELECT stock_code, buy_date FROM low_price_bull_monitored_stocks
                WHERE status = 'holding'
            """)
    ...
    def add_sell_alert(self, stock_code: str, stock_name: str, alert_type: str,
        ...
        try:
            ...
            # 检查是否已存在相同的提醒
            cursor.execute("""
                SELECT id FROM low_price_bull_sell_alerts
            ...
            cursor.execute("""
                INSERT INTO low_price_bull_sell_alerts
    ...
    def get_pending_alerts(self) -> List[Dict]:
        ...
        try:
            conn = self._connect()
            df = pd.read_sql_query("""
                SELECT * FROM low_price_bull_sell_alerts
                WHERE is_sent = 0
    ...
    def mark_alert_sent(self, alert_id: int):
        ...
        try:
            ...
            cursor.execute("""
                UPDATE low_price_bull_sell_alerts
                SET is_sent = 1
                WHERE id = ?
            """, (alert_id,))
    ...
    def clear_old_alerts(self, days: int = 30):
        ...
        try:
            ...
            cursor.execute("""
                DELETE FROM low_price_bull_sell_alerts
