...
class NewsFlowEngine:
    ...
    def _init_modules(self):
        """初始化所有模块"""
        try:
            from app.news_flow_data import NewsFlowDataFetcher
            self.fetcher = NewsFlowDataFetcher()
        except Exception as e:
            logger.error(f"数据获取模块初始化失败: {e}")
        
        try:
            from app.news_flow_model import NewsFlowModel
            self.model = NewsFlowModel()
        except Exception as e:
            logger.error(f"流量模型模块初始化失败: {e}")
        
        try:
            from app.news_flow_sentiment import SentimentAnalyzer
            self.sentiment = SentimentAnalyzer()
        except Exception as e:
            logger.error(f"情绪分析模块初始化失败: {e}")
        
        try:
            from app.news_flow_agents import NewsFlowAgents
            self.agents = NewsFlowAgents()
        except Exception as e:
            logger.error(f"AI分析模块初始化失败: {e}")
        
        try:
            from app.news_flow_alert import NewsFlowAlertSystem
    ...
    def get_dashboard_data(self) -> Dict:
        ...
        try:
            ...
            # 调度器状态
            try:
                from app.news_flow_scheduler import news_flow_scheduler
